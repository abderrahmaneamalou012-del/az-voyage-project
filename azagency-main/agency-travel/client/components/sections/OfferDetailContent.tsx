...
//         <div className={`w-full lg:w-[340px] flex-shrink-0 ${hasLeftContent || hotels.length > 0 ? "sticky top-8" : ""}`}>
//           <PriceSidebar
...
const OfferDetailContent = () => {
  ...
  const { data: inferredHotels = [] } = useHotelsForOffer({
    offerId: offer?.id ?? id,
    offerSlug,
  });
  ...
  const forcedMainImage =
    offer.mainImage &&
    typeof offer.mainImage !== "string" &&
    "cloudinaryUrl" in offer.mainImage &&
    offer.mainImage.cloudinaryUrl
      ? offer.mainImage.cloudinaryUrl
      : "";

  const mainImage = forcedMainImage || getOfferMainImageUrl(offer);
  ...
  const reservationHotels =
    hotels.length > 0
      ? hotels.map((h) => ({
          ...
          childPrice: h.childPrice,
          childPriceAmount: h.childPriceAmount,
          childPriceBrackets: h.childPriceBrackets?.length
            ? h.childPriceBrackets
            : childrenPricing.map((cp) => ({
                label: cp.label,
                priceAmount: cp.priceAmount,
              })),
        }))
